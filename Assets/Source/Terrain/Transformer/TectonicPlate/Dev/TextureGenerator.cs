...
namespace ProjectRise.Terrain.Transformer.TectonicPlate.Dev
{
    ...
    {
        internal static Texture2D GenerateTextureFromModel(PerlinModel model, TerrainModel terrainModel)
        {
            Texture2D texture = new Texture2D(terrainModel.HorizontalTiles, terrainModel.VerticalTiles);
            ...
            for (int y = 0; y < terrainModel.VerticalTiles; y++)
            {
                ...
                {
                    float sample = Mathf.PerlinNoise((x * model.Lacunarity) + model.XOffset,
        ...
        internal static Texture2D GeneratePlateTextureFromModel(PerlinModel model, TerrainModel terrainModel)
        {
            Texture2D texture = new Texture2D(terrainModel.HorizontalTiles, terrainModel.VerticalTiles);
            Color[] pixels = new Color[terrainModel.HorizontalTiles * terrainModel.VerticalTiles];
            int p = 0;
